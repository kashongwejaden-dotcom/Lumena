  ...
  return (
    <div className="fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
        <h1 style={{ fontSize: '2rem' }}>Gestion des Produits</h1>
      ...
      {/* Settings Section */}
      <div style={{ backgroundColor: 'var(--color-white)', padding: '20px', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-sm)', marginBottom: '30px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
        <div>
              ...
              return (
                <tr key={product.id} style={{ borderBottom: '1px solid var(--color-border)', transition: 'background-color 0.2s ease' }} className="hover-row">
                  ...
                  <td style={{ padding: '15px 20px', textAlign: 'right' }}>
                    <button onClick={() => openModal(product)} style={{ padding: '8px', color: '#3182ce', marginRight: '10px', backgroundColor: '#ebf8ff', borderRadius: 'var(--radius-sm)', transition: 'all 0.2s' }}>
                      <Edit2 size={16} />
                    </button>
      ...
      {isModalOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ backgroundColor: 'var(--color-white)', borderRadius: 'var(--radius-md)', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto', padding: '30px', position: 'relative' }} className="fade-in">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                <div className="form-group">
                  ...
                  <p style={{ margin: '5px 0 0 0', fontSize: '0.8rem', color: 'var(--color-text-light)' }}>
                    Seront affichés: {price ? formatPrice(parseFloat(price), exchangeRate).usd : '$0.00'} / {price ? formatPrice(parseFloat(price), exchangeRate).fc : '0 FC'}
                  </p>
                </div>
                <div className="form-group">
                  ...
                  <select className="form-control" value={category} onChange={(e) => setCategory(e.target.value)}>
                    ...
                    <option value="Accessoires Peigne/Pince">Accessoires (Pinces, etc.)</option>
                  </select>
                </div>
              </div>
